...
if (args.length === 0) return message.reply('Musíš vyhledat nějaký NPM package.');
const query = args.join(' ');
try {
const { body } = await snek.get(`https://registry.npmjs.com/${query.toLowerCase()}`);
// Get the latest version by the dist-tag
...
return this.duration.asSeconds() >= 86400 ? "týd [týdnama], d [dnama]" : "h [hodinama], m [minutama], s [sekundama]";
}
...
if(error.status == 404) return message.channel.send('**Nevyhledal jsem žádný NPM package.**');
console.log(error);
}  
