...
async function play(connection, message, bot) {
	...
	var videoinfo = new Discord.RichEmbed()
		...
		.setDescription(`**Popisek:** ${description}`)
		.setFooter("Dot | v1.5a");
...
module.exports.run = async (bot, message, args) => {
	if (!args) {
			message.channel.send(":x: **Zadej prosím odkaz nebo název hudby.**");
	}

	if (!message.member.voiceChannel) {
			message.channel.send(":x: **Nejsi ve voice chatu!**");
	}
	...
	if(ytlc == false) {
		yts(args, { maxResults: 1, key: process.env.ytkey }, (err, data) => {
			...
			data.map((data) => {
				...
				if(ytcc == true) {
					message.channel.send(":x: **Myslím si, že tohle je na 100% kanál a ne hudba.**");
				}
				
				if (ytpc == true) {;
					message.channel.send(":x: **Toto je playlist, nemůžu ho pustit to nejde.** ;(");
				}
