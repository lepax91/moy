...
exports.run = (client, message, args, ops) => { //Collecting info about command
  ...
  search(args.join(' '), function(err, res) {
    ...
    if (err) return message.channel.send({
      embed: {
        "title": ":warning: Stal se problém, toto kontaktujte Dot Development Project!",
        "color": 0xff2222
      }
    }).then(msg => {
    ...
    if (videos.length == 0) return message.channel.send({
      embed: {
        "title": ":warning: Na tvůj požadavek nemohu najít tuto hudbu!",
        "color": 0xff2222
      }
    }).then(msg => {
      if (config[message.guild.id].delete == 'true') {
    ...
    var title = `⏯️ Vyber jednu hudbu a ${videos.length}`;
